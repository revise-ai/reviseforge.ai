...
const INITIAL_MESSAGES: Message[] = [
  {
    ...
    avatar: "SM",
    content:
      "Hey everyone! Welcome to the channel 🎉 Let's use this space to share study materials and help each other out.",
    timestamp: "9:15 AM",
  ...
  },
  {
    ...
    avatar: "KA",
    content:
      "Thanks for the invite! I've been struggling with chapter 4. Anyone have good notes on cellular respiration?",
    timestamp: "9:22 AM",
    date: "Monday, February 16th",
  },
  {
    ...
    avatar: "AB",
    content:
      "I have great notes on that! I'll upload them to the files section later today.",
    timestamp: "9:45 AM",
    date: "Monday, February 16th",
  },
  {
    ...
    avatar: "AE",
    content:
      "Perfect. Let's also plan a study session this week. How does Thursday evening work for everyone?",
    timestamp: "10:03 AM",
            ...
            >
              <div className="flex -space-x-2">
                {MOCK_MEMBERS.slice(0, 3).map((m) => (
                  <div
                    key={m.id}
                    className={`w-7 h-7 rounded-full ${m.color} flex items-center justify-center text-white text-[10px] font-semibold border-2 border-white`}
                  >
                    {m.initials}
                  </div>
                ))}
            ...
            <div key={group.date}>
              ...
              <div className="space-y-4">
                {group.messages.map((msg) => (
                  <div
                    key={msg.id}
                    className="flex gap-3 group hover:bg-gray-50 rounded-xl px-3 py-2 -mx-3 transition-colors"
                  >
                    <div
                      className={`w-9 h-9 rounded-full ${AVATAR_COLORS[msg.avatar] ?? "bg-gray-400"} flex items-center justify-center text-white text-xs font-semibold shrink-0 mt-0.5`}
                    >
                      {msg.avatar}
                    </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white shrink-0">
          <div className="border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent transition-all">
            <div className="flex items-center gap-1 px-3 pt-2 pb-1 border-b border-gray-100">
              ...
              <button
                className="p-1.5 rounded cursor-pointer hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors w-7 h-7 flex items-center justify-center"
                title="Link"
              >
                <svg
                  className="w-3.5 h-3.5"
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1">
            ...
            {MOCK_MEMBERS.filter((m) => m.status === "online").map((member) => (
              ...
              >
                <div className="relative">
                  <div
                    className={`w-8 h-8 rounded-full ${member.color} flex items-center justify-center text-white text-xs font-semibold`}
                  >
                    {member.initials}
                  </div>
            ...
            {MOCK_MEMBERS.filter((m) => m.status === "away").length > 0 && (
              <>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-2 mt-4 mb-2">
                  Away —{" "}
                  {MOCK_MEMBERS.filter((m) => m.status === "away").length}
                </p>
                {MOCK_MEMBERS.filter((m) => m.status === "away").map(
                  (member) => (
                    <div
                      key={member.id}
                      className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                    >
                      <div className="relative">
                        <div
                          className={`w-8 h-8 rounded-full ${member.color} flex items-center justify-center text-white text-xs font-semibold opacity-70`}
                        >
                          {member.initials}
                        </div>
            ...
            {MOCK_MEMBERS.filter((m) => m.status === "offline").length > 0 && (
              <>
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-2 mt-4 mb-2">
                  Offline —{" "}
                  {MOCK_MEMBERS.filter((m) => m.status === "offline").length}
                </p>
                {MOCK_MEMBERS.filter((m) => m.status === "offline").map(
                  (member) => (
                    <div
                      key={member.id}
                      className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                    >
                      <div className="relative">
                        <div
                          className={`w-8 h-8 rounded-full ${member.color} flex items-center justify-center text-white text-xs font-semibold opacity-40`}
                        >
                          {member.initials}
                        </div>
