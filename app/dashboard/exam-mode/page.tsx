...
interface FillQuestion {
  id: number; question: string; correctAnswer: string; explanation: string; category: string;
}
interface WrittenQuestion {
  id: number; question: string; modelAnswer: string; keyPoints: string[]; category: string;
}
interface ExamData { mcq: MCQQuestion[]; fillInBlank: FillQuestion[]; written: WrittenQuestion[]; }
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-6 px-4">
      ...
      <div className="text-center">
        <h2 className="text-xl font-semibold text-gray-800 mb-1">Building your exam{".".repeat(dots)}</h2>
...
// ─── Ready Screen ─────────────────────────────────────────────────────────────
function ReadyScreen({ label, onStart }: { label: string; onStart: () => void }) {
  return (
    <main className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 py-16">
      <div className="w-full max-w-lg text-center">
        ...
        <p className="text-gray-400 text-sm mb-1 font-medium truncate px-8">{label}</p>
        <p className="text-gray-400 text-sm mb-8">Take a deep breath. Once you start, the timer begins immediately.</p>
        <div className="grid grid-cols-4 gap-3 mb-8">
          {[{ value: "20", label: "MCQ" }, { value: "15", label: "Fill-in" }, { value: "15", label: "Written" }, { value: "45m", label: "Time" }].map((s) => (
        ...
        <div className="bg-gray-900 rounded-2xl p-5 mb-8 text-left">
          <p className="text-sm font-semibold text-white mb-3 flex items-center gap-2"><WarnIcon />Exam Rules</p>
          <ul className="space-y-2">
            {["You must answer each question before moving to the next — no skipping",
              ...
              "No hints. No explanations. Results shown only after you finish"].map((rule, i) => (
        ...
        <button onClick={onStart} className="w-full py-4 bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all text-white font-semibold text-base rounded-2xl shadow-md cursor-pointer">
          Start Exam →
        </button>
        <Link href="/dashboard" className="block mt-4 text-sm text-gray-400 hover:text-gray-600 transition cursor-pointer">← Back to Dashboard</Link>
      </div>
    </main>
  );
}
...
export default function ExamModePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!audioBase64 && !transcript) throw new Error("No recording data found. Please go back and record first.");

        const res = await fetch("/api/generate-exam-recording", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ audioBase64: audioBase64 || null, mimeType: mimeType || null, transcript }),
        });
        if (!res.ok) {
          const text = await res.text();
          const msg  = text.startsWith("{") ? JSON.parse(text).error : "Failed to generate exam from recording";
          throw new Error(msg);
        }
        const data = await res.json();
        setExam(data.exam); setPhase("ready");
  ...
  const generateExam = async (file: File) => {
    setLabel(file.name); setPhase("loading"); setError("");
    try {
  ...
  const startExam = () => {
    setPhase("active"); setSection("mcq"); setQIndex(0);
    setMcqAnswers({}); setFillAnswers({}); setWrittenAnswers({});
    setBlocked(false); setTimedOut(false);
    timeLeftRef.current = TOTAL_SECONDS;
    setTimeLeft(TOTAL_SECONDS); setTimeUsed(0);
    timerRef.current = setInterval(() => {
      timeLeftRef.current -= 1; setTimeLeft(timeLeftRef.current);
      if (timeLeftRef.current <= 0) { clearInterval(timerRef.current!); setTimedOut(true); submitExam(); }
    }, 1000);
  };

  const retakeExam = () => {
    if (timerRef.current) clearInterval(timerRef.current);
    setPhase("modal"); setExam(null); setError("");
  };
  ...
  const handleNext = () => {
    if (!isAnswered()) { setBlocked(true); setTimeout(() => setBlocked(false), 3000); return; }
    setBlocked(false); if (!exam) return;
    if      (section === "mcq")     { if (qIndex < exam.mcq.length - 1) setQIndex(qIndex + 1); else { setSection("fill"); setQIndex(0); } }
    else if (section === "fill")    { if (qIndex < exam.fillInBlank.length - 1) setQIndex(qIndex + 1); else { setSection("written"); setQIndex(0); } }
    else                             { if (qIndex < exam.written.length - 1) setQIndex(qIndex + 1); else submitExam(); }
  };

  const handlePrev = () => {
    setBlocked(false); if (!exam) return;
    if      (section === "mcq" && qIndex > 0)     setQIndex(qIndex - 1);
    else if (section === "fill")  { if (qIndex > 0) setQIndex(qIndex - 1); else { setSection("mcq"); setQIndex(exam.mcq.length - 1); } }
    else if (section === "written") { if (qIndex > 0) setQIndex(qIndex - 1); else { setSection("fill"); setQIndex(exam.fillInBlank.length - 1); } }
  };

  const totalQ       = exam ? exam.mcq.length + exam.fillInBlank.length + exam.written.length : 50;
  const overallIndex = !exam ? 0 : section === "mcq" ? qIndex : section === "fill" ? exam.mcq.length + qIndex : exam.mcq.length + exam.fillInBlank.length + qIndex;
  const answeredCount = Object.keys(mcqAnswers).length + Object.keys(fillAnswers).length + Object.keys(writtenAnswers).length;
  ...
  if (phase === "modal") return (
    <>
      {error && (<div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 text-red-600 rounded-xl text-sm font-medium shadow-lg"><XIcon cls="w-4 h-4 text-red-500" />{error}</div>)}
      <ExamModeModal show={true} onClose={() => {}} onReady={(file) => generateExam(file)} />
    </>
  );

  if (phase === "loading") return (
    <LoadingScreen label={
      isRecordingSource ? "Analysing your recording…" :
      isYoutubeSource   ? `Analysing ${label}` :
      "Building your exam…"
    } />
  );
  ...
  if (phase === "ready") return (
    <ReadyScreen label={
      isRecordingSource ? "Generated from your recording" :
      label
    } onStart={startExam} />
  );

  if (phase === "done" && exam) return (
    <ResultsScreen exam={exam} mcqAnswers={mcqAnswers} fillAnswers={fillAnswers} writtenAnswers={writtenAnswers}
      timeUsed={timeUsed} timedOut={timedOut} onRetake={retakeExam}
      ...
      <div className="min-h-screen bg-gray-50">
        <header className="sticky top-0 z-20 bg-white border-b border-gray-200 px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-sm text-gray-400 font-medium shrink-0">{overallIndex + 1}<span className="text-gray-300 mx-0.5">/</span>{totalQ}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 hidden sm:block">{answeredCount} answered</span>
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-mono font-bold text-sm tabular-nums transition-colors ${timerDanger ? "bg-red-100 text-red-600 animate-pulse" : timerWarn ? "bg-yellow-50 text-yellow-600" : "bg-gray-100 text-gray-700"}`}>
              <ClockIcon />{fmt(timeLeft)}
            </div>
            <button onClick={submitExam} className="text-xs px-3 py-1.5 border border-gray-200 hover:bg-gray-50 text-gray-500 rounded-lg transition cursor-pointer hidden sm:block">Submit</button>
          </div>
        </header>
        <div className="h-0.5 bg-gray-200"><div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${((overallIndex + 1) / totalQ) * 100}%` }} /></div>
        {blocked && (<div className="max-w-2xl mx-auto mt-4 px-4"><div className="flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600 font-medium"><WarnIcon />You must answer this question before moving to the next one.</div></div>)}
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          {section === "mcq" && (() => {
            const q = exam.mcq[qIndex]; const sel = mcqAnswers[q.id];
            return (
              <div className="bg-white rounded-3xl border border-gray-200 shadow-sm">
                ...
                <div className="px-6 pb-3"><p className="text-lg font-medium text-gray-900 leading-relaxed">{q.question}</p></div>
                <div className="px-6 pb-7 space-y-3">
                  {(["A","B","C","D"] as const).map((key) => (
                    <button key={key} onClick={() => setMcqAnswers((p) => ({ ...p, [q.id]: key }))}
                      ...
                      <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 transition-colors ${sel === key ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-500"}`}>{key}</span>
          ...
          {section === "fill" && (() => {
            const q = exam.fillInBlank[qIndex]; const val = fillAnswers[q.id] ?? ""; const parts = q.question.split("[BLANK]");
            return (
              <div className="bg-white rounded-3xl border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between px-6 pt-5 pb-2">
                ...
                <div className="px-6 pb-3">
                  <p className="text-lg font-medium text-gray-900 leading-relaxed">
                    {parts[0]}<span className="inline-block mx-1.5 px-3 py-0.5 bg-blue-50 border-b-2 border-blue-400 text-blue-700 rounded text-base font-semibold min-w-[80px] text-center">{val || "___"}</span>{parts[1]}
                  </p>
                </div>
                <div className="px-6 pb-7">
                  <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Your Answer</label>
                  <input type="text" value={val} onChange={(e) => setFillAnswers((p) => ({ ...p, [q.id]: e.target.value }))} onKeyDown={(e) => e.key === "Enter" && handleNext()}
                    placeholder="Type the exact term or phrase..." className="w-full px-4 py-3 border-2 border-gray-200 focus:border-blue-500 rounded-xl text-sm text-gray-900 outline-none transition placeholder-gray-300" autoFocus />
          ...
          {section === "written" && (() => {
            const q = exam.written[qIndex]; const val = writtenAnswers[q.id] ?? ""; const words = val.trim().split(/\s+/).filter(Boolean).length;
            return (
              <div className="bg-white rounded-3xl border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between px-6 pt-5 pb-2">
                ...
                <div className="px-6 pb-7">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-medium ${words >= 30 ? "text-green-600" : "text-gray-400"}`}>{words} {words === 1 ? "word" : "words"}</span>
                  </div>
                  <textarea value={val} onChange={(e) => setWrittenAnswers((p) => ({ ...p, [q.id]: e.target.value }))} rows={9}
                    placeholder="Write a thorough, detailed answer. Aim for at least 3–5 sentences..."
                    className="w-full px-4 py-3 border-2 border-gray-200 focus:border-blue-500 rounded-xl text-sm text-gray-900 outline-none transition resize-none placeholder-gray-300 leading-relaxed" autoFocus />
          ...
          <div className="flex items-center justify-between mt-6">
            <button onClick={handlePrev} disabled={section === "mcq" && qIndex === 0}
            ...
            <div className="flex gap-1 flex-wrap justify-center max-w-[180px]">
              {Array.from({ length: totalQ }).map((_, i) => {
                ...
                const isAns = i < exam.mcq.length ? mcqAnswers[exam.mcq[i]?.id] !== undefined
                  : i < exam.mcq.length + exam.fillInBlank.length ? (fillAnswers[exam.fillInBlank[i - exam.mcq.length]?.id] ?? "").trim().length > 0
                  : (writtenAnswers[exam.written[i - exam.mcq.length - exam.fillInBlank.length]?.id] ?? "").trim().length > 10;
                return <div key={i} className={`h-1.5 rounded-full transition-all ${isCurrent ? "w-3 bg-blue-600" : isAns ? "w-1.5 bg-gray-400" : "w-1.5 bg-gray-200"}`} />;
              })}
            </div>
