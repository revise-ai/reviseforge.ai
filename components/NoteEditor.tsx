...
export default function NoteEditor({
  ...
  // Anchor node & offset where the FIRST voice session started in this editor
  // so every new session appends right after previous transcript
  const voiceInsertRangeRef = useRef<Range | null>(null);
  const mediaRecorderRef = useRef<MediaRecorder | null>(null);
  const audioChunksRef = useRef<Blob[]>([]);
  // Whether we already placed the cursor for this voice session chain
  const voiceAnchoredRef = useRef(false);
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // ── VOICE: Gemini-powered recording ──────────────────────────────────────
  //
  // Flow:
  //   Click Voice (first time or after Stop) → record audio via MediaRecorder
  //   Click Stop                             → send audio to /api/transcribe
  //                                            → insert cleaned transcript at
  //                                              the same anchor position
  //   Click Voice again                      → continue recording, result will
  //                                            be appended right after the last
  //                                            transcript (voiceInsertRangeRef)
  //
  // The anchor (voiceInsertRangeRef) is set once when the user first clicks
  // Voice, pointing to wherever their cursor was.  After each transcript is
  // inserted the anchor is moved to the end of that new text, so the next
  // session appends seamlessly.
  // ─────────────────────────────────────────────────────────────────────────

  const handleVoice = async () => {
    // ── STOP ──
    if (isListening) {
      mediaRecorderRef.current?.stop(); // triggers onstop → sends to Gemini
      setIsListening(false);
      return;
    }
    ...
    // Pin the insert position the very first time in this chain
    if (!voiceAnchoredRef.current) {
      ensureCursor();
      const sel = window.getSelection();
      if (sel && sel.rangeCount > 0 && editorRef.current?.contains(sel.anchorNode)) {
        voiceInsertRangeRef.current = sel.getRangeAt(0).cloneRange();
      } else {
        // Fallback: end of editor
        const r = document.createRange();
        r.selectNodeContents(editorRef.current!);
        r.collapse(false);
        voiceInsertRangeRef.current = r;
      }
      voiceAnchoredRef.current = true;
    }
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const res = await fetch("/api/transcribe", {
          ...
          body: JSON.stringify({
            audio: base64,
            mimeType: mimeType.split(";")[0], // strip codec hint for Gemini
          }),
        });
        ...
        if (
          ...
          // Restore the anchor range
          const sel = window.getSelection();
          sel?.removeAllRanges();
          if (voiceInsertRangeRef.current) {
            sel?.addRange(voiceInsertRangeRef.current.cloneRange());
          }

          // Insert a space separator if there's already content at the anchor
          const range = sel?.getRangeAt(0);
          if (range) {
            // Add a space before transcript if needed
            ...
            // Move anchor to end of inserted text for next session
            ...
            // Save anchor for next voice session continuity
            voiceInsertRangeRef.current = newRange.cloneRange();
  ...
  // Reset voice anchor when user manually moves cursor away (clicks elsewhere)
  // so next Voice click anchors to the new cursor position
  useEffect(() => {
    const handleEditorClick = () => {
      if (!isListening) {
        voiceAnchoredRef.current = false;
        voiceInsertRangeRef.current = null;
      }
    };
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="flex items-center gap-0.5 px-3 py-1 bg-white border-b border-gray-100 shrink-0 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2">
          {/* Voice button — UI unchanged, now powered by Gemini */}
          <button
            onClick={handleVoice}
            disabled={isTranscribing}
            className={`cursor-pointer flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl transition-all ${
              isListening
          ...
          >
            ...
            {isListening ? "Stop" : isTranscribing ? "Processing…" : "Voice"}
          </button>
          <button
            onClick={handleAIPolish}
            className="cursor-pointer flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-semibold rounded-xl shadow-sm"
      ...
      >
        <div className="max-w-3xl mx-auto">
          ...
          {isTranscribing && (
            <div className="mb-4 flex items-center gap-2 px-4 py-2.5 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-600">
