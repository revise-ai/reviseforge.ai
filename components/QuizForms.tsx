      ...
      {uploadingFiles.length > 0 && (
        <div className="fixed top-5 right-5 z-[200] w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
            ...
            <button onClick={() => setPanelCollapsed((c) => !c)} className="text-gray-400 hover:text-gray-600 transition cursor-pointer">
                ...
                return (
                  <div key={f.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="shrink-0">
                      {f.done ? (
                        <button onClick={() => removeFile(f.id)} className="text-gray-300 hover:text-red-400 transition cursor-pointer">
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
          ...
          <div className="px-8 pb-8">
            ...
            >
                  ...
                  {doneFiles.map((f) => {
                    ...
                    return (
                      <div key={f.id} className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2">
                        ...
                        <button type="button" onClick={(e) => { e.preventDefault(); removeFile(f.id); }}
              ...
              {/* In-progress files inside dropzone */}
              {inProgressFiles.length > 0 && (
              ...
              <input id="quiz-file-input" type="file" multiple accept=".pdf,.doc,.docx,.txt,.ppt,.pptx"
            ...
            <div className="mt-6 flex justify-end gap-4">
              ...
              <button type="button" onClick={handleGenerate} disabled={!hasDone || loading}
                className={`px-6 py-2 active:scale-95 transition-all text-white rounded-lg flex items-center gap-2 ${
                  !hasDone || loading ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 cursor-pointer"
                }`}>
                {loading ? (
                  <>
                ...
                ) : !hasDone ? "Add Resources First" : "Generate Quiz"}
              </button>
            </div>
          </div>
        </div>
